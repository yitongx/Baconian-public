...
class DDPG(ModelFreeAlgo, OffPolicyAlgo, MultiPlaceholderInput):
    ...
    def __init__(self,
        ...
        MultiPlaceholderInput.__init__(self,
                                       sub_placeholder_input_list=[dict(obj=self.target_actor, attr_name='target_actor',),
                                                                   dict(obj=self.target_critic, attr_name='target_critic'),
                                                                   dict(obj=self.actor, attr_name='actor'),
                                                                   dict(obj=self.critic, attr_name='critic')],
    ...
    def train(self, batch_data=None, train_iter=None, sess=None, update_target=True) -> dict:
        ...
        for i in range(train_iter):
            train_batch = self.replay_buffer.sample(batch_size=self.parameters('BATCH_SIZE')) \
                if batch_data is None else batch_data
    ...
    def predict_with_noise(self, obs:np.ndarray, sess=None, batch_flag: bool=False, noise_scale: float=0.5, **kwargs):
        '''
        Input single or list obs, predict action with noise.
        :param obs:
        :param sess:
        :param batch_flag:
        :param noise_scale:
        ...
        action = self.actor.forward(obs=obs, sess=tf_sess, feed_dict=feed_dict)
        noise = noise_scale * np.random.randn(np.prod(action.shape))
        noise = make_batch(noise, [action.shape[-1]])
        assert action.shape == noise.shape
        action += noise
        high_bound = self.env_spec.action_space.high if not 'high_bound' in kwargs else kwargs['high_bound']
        low_bound = -high_bound if not 'low_bound' in kwargs else kwargs['low_bound']
        action = np.clip(action, a_min=low_bound, a_max=high_bound)
        return action
    ...
    def _set_up_actor_loss(self):
        ...
        optimize_op = optimizer.apply_gradients(grad_var_pair)
        op = []
        for var, target_var in zip(self.actor.parameters('tf_var_list'), self.target_actor.parameters('tf_var_list')):
            ref_val = self.parameters('DECAY') * target_var + (1.0 - self.parameters('DECAY')) * var
            op.append(tf.assign(target_var, ref_val))
