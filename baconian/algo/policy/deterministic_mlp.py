...
class DeterministicMLPPolicy(DeterministicPolicy, PlaceholderInput):
    ...
    def forward(self, obs: (np.ndarray, list), sess=None, feed_dict=None, **kwargs):
        ...
        act_low_bound = self.env_spec.action_space.low if not 'act_low_bound' in kwargs else kwargs['act_low_bound']
        act_high_bound = self.env_spec.action_space.high if not 'act_high_bound' in kwargs else kwargs['act_high_bound']
        assert len(act_low_bound) == 1 and len(act_high_bound) == 1
        res = np.clip(res, a_min=act_low_bound, a_max=act_high_bound)
        return res
