...
class AgentActionNoiseWrapper(object):
    ...
    @typechecked
    def __init__(self, noise: ActionNoise,
                 action_weight_scheduler: Scheduler,
                 noise_weight_scheduler: Scheduler):
        self.action_weight_scheduler = action_weight_scheduler
        self.noise_weight_scheduler = noise_weight_scheduler
        self.noise = noise
        self.noise_recored_decay = 1.0

    def __call__(self, action, **kwargs):
        noise = self.noise()
        self.noise_recored_decay = self.noise_recored_decay * self.noise_weight_scheduler.value()
        return self.action_weight_scheduler.value() * action + self.noise_recored_decay * noise
