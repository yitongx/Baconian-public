...
class _SingletonLogger(BaseLogger):
    ...
    def _save_all_obj_final_status(self):
        ...
        ConsoleLogger().print('info', 'save final_status into {}'.\
                              format(os.path.join(self._record_file_log_dir)))
        self.out_to_file(file_path=os.path.join(self._record_file_log_dir),
                         content=final_status,
                         force_new=True,
                         file_name='final_status.json')
        ConsoleLogger().print('info', 'save global_config into {}'.\
                              format(os.path.join(self._record_file_log_dir)))
        self.out_to_file(file_path=os.path.join(self._record_file_log_dir),
...
def reset_global_recorder():
    globals()['_global_recorder'].reset()   # reset recorder in globals()
