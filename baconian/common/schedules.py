...
from baconian.core.global_var import Singleton
class DDPGNoiseScheduler(Scheduler):
    def __init__(self, train_every_step,
                 train_after_step,
                 noise_decay,
                 step_counter: Singleton):
        self.train_every_step = train_every_step
        self.train_after_step = train_after_step
        self.noise_decay = noise_decay
        self.step_counter = step_counter
    def value(self):
        if self.step_counter.val > self.train_after_step and \
                self.step_counter.val % self.train_every_step == 0:
            return self.noise_decay
        else:
            return 1.0
...
if __name__ == '__main__':
    a = Scheduler()
    b = ConstantScheduler(3)
    assert isinstance(b, Scheduler)
    assert isinstance(b, ConstantScheduler)