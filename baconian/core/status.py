...
class StatusWithSingleInfo(StatusWithInfo):
    ...
    def get_specific_info_key_status(self, info_key, *args, **kwargs):
        try:
            ...
            ConsoleLogger().print('ERROR', 'try to access info key status: {} of obj: {}'.format(info_key, self.obj.name))
            return None
...
class StatusWithSubInfo(StatusWithInfo):
    ...
    # access to obj._info_dict_with_sub_info through group visit, defined as total_step_count_fn()
...
def register_counter_info_to_status_decorator(increment, info_key, under_status: (str, tuple) = None,
    ...
    def wrap(fn):
        ...
        def wrap_with_self(self, *args, **kwargs):
            ...
            obj = self
            # print('obj in register_counter_info_to_status_decorator:', obj, '\n',
                  # '\n'.join(['%s:%s' % item for item in obj.__dict__.items()]))

            if not hasattr(obj, '_status') or not isinstance(getattr(obj, '_status'), StatusWithInfo):
                raise ValueError(' the object {} does not not have attribute StatusWithInfo instance \
                or hold wrong type of Status'.format(obj))
            ...
            res = fn(self, *args, **kwargs) # return the func that being decorated in agent.py

            for st in final_st:
                if st and st != obj.get_status()['status'] and not ignore_wrong_status:
                    raise ValueError('register counter info under status: {} but got status {}'.\
                                     format(st,obj.get_status()['status']))
