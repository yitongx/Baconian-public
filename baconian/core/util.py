...
def register_name_globally(name: str, obj):
    if name in get_all()['_global_name_dict'] \
            and not id(obj) == id(get_all()['_global_name_dict'][name]) \
            and obj.allow_duplicate_name is False \
            and get_all()['_global_name_dict'][name].allow_duplicate_name is False \
            and GlobalConfig().DEFAULT_TURN_OFF_GLOBAL_NAME_FLAG is False:
